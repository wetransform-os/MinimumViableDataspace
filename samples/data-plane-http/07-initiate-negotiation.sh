...
OFFER=$(curl -X 'POST' \
  'http://localhost:9192/api/v1/data/catalog/request' \
  -H 'accept: application/json' \
  -H 'X-Api-Key: ApiKeyDefaultValue' \
  -H 'Content-Type: application/json' \
  -d '{"providerUrl": "http://company1:8282/api/v1/ids/data"}' | jq '.contractOffers[]  | select(.asset.id == "assetId")')
...
REQUEST=$(cat <<EOF
{
  ...
  "connectorAddress": "http://company1:8282/api/v1/ids/data",
...
NEGOTIATION=$(curl -d "$REQUEST" -X POST -H 'content-type: application/json' -H 'X-Api-Key: ApiKeyDefaultValue' http://localhost:9192/api/v1/data/contractnegotiations \
         -s)

NEGOTIATION_ID=$(echo $NEGOTIATION | jq -r .id)
echo "Negotiation ID: $NEGOTIATION_ID"

NEGOTIATION=$(curl -X GET "http://localhost:9192/api/v1/data/contractnegotiations/$NEGOTIATION_ID" \
...
curl -X POST "http://localhost:9192/api/v1/data/transferprocess" \
